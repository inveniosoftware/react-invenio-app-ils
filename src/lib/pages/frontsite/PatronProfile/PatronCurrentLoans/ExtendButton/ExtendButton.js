...
export default class ExtendButton extends Component {
  ...
  validateExtendUrl() {
    ...
    return _has(loan, 'availableActions.extend');
  }

  validateExtendAction() {
    const { loan } = this.props;

    return _get(loan, 'metadata.end_date').diffNow('days').days <= expireDays;
  }
