...
export const MetadataExtensions = ({ extensions }) => {
  const { label, fields } = extensionsConfig.document;
  ...
  _forOwn(allExtensions, (value, key) => {
    const componentType = extensionsConfig.document.fields[key].type;
    ...
    components.push(
      <Component
        ...
        key={key}
        label={_get(extensionsConfig, `document.fields.${key}.label`, key)}
        optimized
        required={_get(
          extensionsConfig,
          `document.fields.${key}.isRequired`,
          false
        )}
      />
    );
